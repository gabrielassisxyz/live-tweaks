...
- **`/tweaks`** (no argument) — **setup mode**: inventory the app's CSS custom
  properties (design tokens), write or refresh `.live-tweaks/design-tokens.md`, then print
...
### Step 1 — Locate the repo root and `.live-tweaks/design-tokens.md`

The repo root is the nearest ancestor directory containing a `.git` entry,
starting from the current working directory. The token inventory (if it exists)
lives at `.live-tweaks/design-tokens.md`, relative to that root.

The tool namespaces itself in a `.live-tweaks/` directory rather than dropping a
file at the repo root on purpose: the root is contested space, and a generic
name there collides with — and gets mistaken for — the hand-written design docs
many repos already keep (`DESIGN.md`). A dedicated directory makes the owner
obvious from the path alone and leaves room for future artifacts without
claiming another root filename.

- If `.live-tweaks/design-tokens.md` is present, read it in full — it carries a scan manifest
...
`.live-tweaks/design-tokens.md` is a **cache**, not a source of truth — never trust it blindly.
...
`.live-tweaks/design-tokens.md` opens with an HTML-comment scan manifest, one line per file that
...
3. Otherwise → **fresh**. Skip Step 3, use the existing inventory, and go
   straight to Step 4 with the inventory you just read.
...
### Step 3 — Rescan (missing or stale `.live-tweaks/design-tokens.md`)
...
#### 3f. Write `.live-tweaks/design-tokens.md`, wholesale

Overwrite `.live-tweaks/design-tokens.md` completely — never edit it in place,
never append. Create the `.live-tweaks/` directory at the repo root if it does
not exist. Use this shape:
...
**Commit it — do not add `.live-tweaks/` to `.gitignore`.** The inventory is a
cache, but a deterministic one derived from committed CSS, so it does not drift
per machine. Tracking it buys two things ignoring it cannot: the diff of a PR
that adds or recolors a token becomes visible review signal, and a fresh clone
starts with a valid cache instead of paying for a rescan. If the user
nonetheless prefers to ignore it, that is a supported choice — the skill just
rescans every time. Say so rather than deciding for them.

**Safety.** `.live-tweaks/design-tokens.md` is the only file this mode writes,
and it always writes inside the repo root determined in Step 1 — never outside
it, and never any other file. It contains plain markdown data (names, values, paths); no
...
Whether `.live-tweaks/design-tokens.md` was reused (Step 2: fresh) or just rewritten (Step 3),
...
defines `--color-*`), so build the allowlist from `.live-tweaks/design-tokens.md`'s **exact token
...
<script>
  window.LiveTweaksConfig = {
    allow: ["--color-bg-base", "--font-headline" /* …every inventoried token name */],
  };
</script>
```
...
tokens, write `.live-tweaks/design-tokens.md`, and reach Step 4 to print the panel-injection
instructions. The no-vars path has done its job the moment there is at least
one `:root`-level custom property to edit.

**Safety.** Unlike the token-having path (which writes only `.live-tweaks/design-tokens.md`), this
...
Reuse setup mode's scan: if a `.live-tweaks/design-tokens.md` is present and validates as **fresh**
...
authority, `.live-tweaks/design-tokens.md` is only the index that points you at the candidates.
